...
def register_l10_routes(app):
    ...
    def add_l10_meeting(division_id):
        ...
        if request.method == 'POST':
            ...
            # Standard L10 sections - 60 minute meeting
            # Order: Segue, Headlines, Scorecard, Rock Review, To-Do, IDS, Conclude
            standard_sections = [
                ('Segue', 1, 5),
                ('Headlines', 2, 5),
    ...
    def view_l10_meeting(division_id, meeting_id):
        ...
        # Rocks (all active for this division)
        cursor.execute("""
    ...
    def start_l10_meeting(division_id, meeting_id):
        ...
        """, (meeting_id, division_id))
        log_action(user['id'], 'l10_meetings', meeting_id, 'UPDATE',
                   changes={'status': 'IN_PROGRESS'}, organization_id=1,
                   division_id=division_id, ip_address=request.remote_addr)
        conn.commit()
        conn.close()
    ...
    def complete_l10_meeting(division_id, meeting_id):
        ...
        cursor.execute("SELECT started_at FROM l10_meetings WHERE id = ? AND division_id = ?",
                       (meeting_id, division_id))
        meeting = cursor.fetchone()
    ...
    # =========================================================
    # AJAX API - AUTO-SAVE ENDPOINTS
    # =========================================================

    @app.route('/api/l10/<int:meeting_id>/save-notes', methods=['POST'])
    @login_required
    def l10_save_notes(meeting_id):
        """Save meeting-level notes (segue, headlines, scorecard_review, rock_review)"""
        ...
        cursor = conn.cursor()
        cursor.execute(f"""
            UPDATE l10_meetings SET {field} = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        """, (value, meeting_id))
        conn.commit()
        conn.close()
    ...
    def l10_save_section(meeting_id):
        ...
        cursor.execute(f"""
            UPDATE l10_sections SET {', '.join(update_fields)}
    ...
    def l10_update_rock(rock_id):
        """Inline update a rock from the L10 meeting"""
        data = request.get_json()
        conn = get_db()
        cursor = conn.cursor()
        ...
        cursor.execute(f"UPDATE rocks SET {', '.join(updates)} WHERE id = ?", params)
        conn.commit()
        conn.close()
        return jsonify({'success': True})
    ...
    def l10_update_todo(todo_id):
        """Inline update a todo - mark done, edit task, etc."""
        data = request.get_json()
        conn = get_db()
        cursor = conn.cursor()
        ...
        for field in ['task', 'owner', 'due_date', 'status']:
            if field in data and field != 'is_completed':
                cursor.execute(f"UPDATE todos SET {field} = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                               (data[field], todo_id))
        conn.commit()
        conn.close()
        return jsonify({'success': True})
    ...
    @login_required
    def l10_create_todo():
        """Create a new todo from L10 meeting"""
        data = request.get_json()
        user = session.get('user')
        conn = get_db()
        cursor = conn.cursor()
    ...
    def l10_update_issue(issue_id):
        """Inline update an issue - change status/priority, add notes, resolve"""
        data = request.get_json()
        conn = get_db()
        cursor = conn.cursor()
    ...
    def l10_create_issue():
        """Create a new issue from L10 meeting"""
        data = request.get_json()
        user = session.get('user')
        conn = get_db()
        cursor = conn.cursor()
    ...
    def l10_complete_with_email(meeting_id):
        ...
        cursor.execute("SELECT started_at, meeting_date, division_id FROM l10_meetings WHERE id = ?", (meeting_id,))
        meeting = cursor.fetchone()
        if not meeting:
            conn.close()
        ...
        # Gather summary data
        cursor.execute("SELECT d.display_name FROM divisions d WHERE d.id = ?", (division_id,))
        div_row = cursor.fetchone()
        div_name = div_row['display_name'] if div_row else 'Division'

        cursor.execute("SELECT segue_good_news, customer_employee_headlines, scorecard_review, rock_review FROM l10_meetings WHERE id = ?", (meeting_id,))
        m = dict(cursor.fetchone())

        # Get todos created during this meeting
        cursor.execute("SELECT task, owner FROM todos WHERE source_l10_id = ? AND is_active = 1", (meeting_id,))
        new_todos = [dict(r) for r in cursor.fetchall()]
